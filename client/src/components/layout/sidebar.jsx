...
import { LayoutDashboard, Users, TicketIcon, UserCog, Settings } from 'lucide-react';
var SidebarItem = function (_a) {
    var href = _a.href, icon = _a.icon, label = _a.label, isActive = _a.isActive;
    return (<Link href={href}>
      <div className={cn("sidebar-item flex items-center px-4 py-3 rounded-md mb-1 cursor-pointer", isActive
            ? "active"
...
export var Sidebar = function (_a) {
    ...
    var filteredNavItems = navItems.filter(function (item) {
        if (!user || !item.roles)
            return false;
    ...
    return (<>
      ...
      <div className="w-64 bg-white border-r border-neutral-200 flex-shrink-0 hidden md:block">
        ...
        <nav className="p-4">
          {filteredNavItems.map(function (item) { return (<SidebarItem key={item.href} href={item.href} icon={item.icon} label={item.label} isActive={item.href === "/"
                ? currentPath === "/"
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-neutral-200 md:hidden">
        <nav className="flex justify-around p-2">
          {filteredNavItems.map(function (item) { return (<a key={item.href} href={item.href} className={cn("flex flex-col items-center p-2 rounded-md", (item.href === "/"
                ? currentPath === "/"
                : currentPath.startsWith(item.href))
                ? "text-primary"
                : "text-neutral-700")}>
